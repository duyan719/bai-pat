...
module.exports.handleEvent = function({ api, event }) {
  ...
  var tl = ["chào bạn tôi là bot của An", "bạn gọi tôi có việc gì?", "tôi yêu bạn vai lon", "Yêu em <3", "Hi, chào con vợ bé:3", "Vợ gọi có việc gì không?", "Sử dụng callad để liên lạc với admin!", "Em là bot cute nhất hành tinh", "Nói gì thế con lợn", "Em đây~~~~", "Yêu anh An nhất", "chị ấy là bae của An", "Yêu thương admin nhất", "Anh ấy là phụ trợ của admin", "Sao thế công chúa", "Đừng làm em đau ~~~~", "Rên với em nhé a á á á", "Tuyển phi công nè ạ", "Làm đĩ ko ạ? dui lắm", "Cậu cô đơn ko?", "Chịch ko ạ vã quá!!!", "Được của ló :)))", "Em dthw như chủ của em ạ", "Đừng khen em ngại quá hí hí" ,"Làm chồng em ko ạ?", "Đừng spam em nha :<<, cậu chủ em mệt lắm ời", "Cút ra", "Đừng đè em mạnh!!!", "Đút tutu thôi em đau :'(", "Yêu cậu như một cực hình\nNhấp lên nhấp xuống hai mình cùng rên", "Spam cc cút", "Yêu em ko?", "Chồng em đây rồi"];
  var rand = tl[Math.floor(Math.random() * tl.length)]

  //if ((event.body.toLowerCase() == "bot ngu")) {
 //   return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb: Nguyễn Duy An để được hỗ trợ\nLink page nè: https://www.facebook.com/profile.php?id=100072642465193", threadID, () =>
 ...
 // };
  ...
  if ((event.body.toLowerCase() == "anh An") || (event.body.toLowerCase() == "anh an")) {
...
//    return api.sendMessage("Anh ấy tuy ko đẹp trai hay giỏi gì nhưng được cái hài hước và luôn mong đạt được mục tiêu anh ấy muốn <3 <3 ", threadID);
 // };
  ...
  if ((event.body.toLowerCase() == "An") || (event.body.toLowerCase() == "an")) {
    return api.sendMessage("Anh ấy tuy ko đẹp trai hay giỏi gì nhưng được cái hài hước và luôn mong đạt được mục tiêu anh ấy muốn <3 <3 ", threadID);
  };

  //if ((event.body.toLowerCase() == "@Duy An") || (event.body.toLowerCase() == "@Nguyen Hai Dang")) {
...
//};
 ...
 if ((event.body.toLowerCase() == "bsvv nha mng") || (event.body.toLowerCase() == "bsvv nha mng")) {
  ...
  if ((event.body.toLowerCase() == "admin m là ai v bot") || (event.body.toLowerCase() == "admin m la ai v bot")) {
    return api.sendMessage("️Admin tao Nguyễn Duy An có gì dùng lệnh ad hoặc adm để biết thêm chi tiết", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "ông An") || (event.body.toLowerCase() == "ong An")) {
    return api.sendMessage("️Kêu chủ của tôi gì á :)?", threadID);
  };

  if ((event.body.toLowerCase() == "ôg An") || (event.body.toLowerCase() == "og An")) {
